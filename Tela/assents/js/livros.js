...
function limparCampos() {
    ...
    fetch('http://localhost:8080/api/dados_livro', {
      method: 'GET',
      headers: {
...
function carregarStatus() {
    fetch('http://localhost:8080/api/dados_livro/status')
        .then(response => response.json())
        .then(data => {
...
function carregarGeneros() {
    fetch('http://localhost:8080/api/dados_livro/generos')
        .then(response => {
            if (!response.ok) {
...
.catch(error=> {
  ...
  function addlinha(dadosAPI){
      ...
      dadosAPI.forEach(element => {   
        ...
        <tr>
            ...
            <button class="bg-green-500 text-black px-2 py-1 rounded" onclick="editar(this, event, ${element.id})">Editar</button>
        </td>
  </tr>
        `;
 ...
 function cadastrar() {
    ...
    function remover(dadosbotao,id){
      ...
      titulo.remove();
      fetch(`http://localhost:8080/api/dados_livro/${id}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json'
      },
...
function editar(dadosbotao, event, id) {
  ...
  Promise.all([
    fetch('http://localhost:8080/api/dados_livro/generos').then(res => res.json()),
    fetch('http://localhost:8080/api/dados_livro/status').then(res => res.json()),
    fetch('http://localhost:8080/api/bibliotecario').then(res => res.json()) 
  ])
